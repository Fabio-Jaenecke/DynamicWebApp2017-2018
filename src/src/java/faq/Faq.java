...
public class Faq {
  ...
  
  {
    ...
    qna.put(1, new QnA("Gibt es Labortests zur Diagnose der Histaminunverträglichkeit?",
        "Nein, bis heute fehlen geeignete Labortests zur Diagnose einer Histaminunverträglichkeit. Angebotene Tests zur Messung des Diaminoxidasespiegels im Blut oder auch die Bestimmung von Histamin im Blutplasma oder Urin sind wenig aussagekräftig und werden nicht empfohlen. Das gleiche gilt für den sogenannten Histamin 50 Prick-Test. Da diese Tests keine verlässlichen Aussagen zulassen, werden sie auch meistens nicht von den Krankenkassen erstattet."));
    qna.put(2, new QnA("Kann man das Histaminintoleranzsyndrom mit Medikamenten behandeln?",
        "Wenn eine konsequente Ernährungsumstellung nicht möglich ist, können Medikamente eingenommen werden, die an die Histaminrezeptoren H1 und H2 binden und diese blockieren. Zu diesen als Antihistaminika bezeichneten Medikamenten zählen zum Beispiel Ceterizin und Cimetidin. Auch Medikamente, die die Aktivität der Diaminooxidase (DAO) fördern, haben in Studien gute Effektivität in der Behandlung des HIS gezeigt (z.B. DAOSiN). Wie wichtig die DAO für den Menschen ist zeigen Studien, bei denen die DAO in ihrer Funktion gezielt eingeschränkt wird. Ist die DAO nicht mehr aktiv und kommt es zu einer Aufnahme grosser Mengen Histamin, kann es zu lebensbedrohlichen Körperreaktionen kommen, wie zum Beispiel Blutdruckabfall, Luftnot und Schock."));
    qna.put(3, new QnA("Wie funktioniert DAOSiN?",
        "Durch den Verzehr der DAOSiN-Kapsel vor dem Essen wird die DAO-Menge im Dünndarm und somit die Fähigkeit zum Histaminabbau durch die natürliche DAO erhöht. Dadurch kann ein ausreichender Abbau von Histamin aus Lebensmitteln im Verdauungstrakt erzielt werden."));
    qna.put(4, new QnA(
