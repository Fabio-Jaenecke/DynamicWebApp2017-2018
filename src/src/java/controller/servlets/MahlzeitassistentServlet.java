...
public class MahlzeitassistentServlet extends HttpServlet {
  ...
  /**
   * Das Servlet ruft standardmaessig den Mahlzeitassistent auf, um ihn zu oeffnen. Sollte der Benutzer auf eine Auswahl klicken,
   * wird Mahlzeitassistentenauswahl geoeffnet Mit einer auswahl in Mahlzeitassisten kehrt der Benutzer wieder zu Mahlzeitassistent
   * zurueck Else, falls kein richtiger Paramter gefunden wurde, kehre zur Home-Seite zurueck.
   * 
  ...
  /**
   * Diese Methode schaut, welches Lebensmittel in der Tabelle ausgewaehlt wurde und gleicht dieses mit dem Attribut in der gleichen
   * Session ab. Auswahl ist dabei ein Lebensmittelname, welcher zu einen in der Lebensmittelliste vorkommt und zu anderen als
   * Parameter weitergegeben wird.
   * 
   * @param request
   * @param response
   * @return
   ...
   * @throws IOException
   */
  protected void setzeKategorienZurueck(HttpServletRequest request, HttpServletResponse response)
   ...
   * @throws IOException
   */
  protected void setzeNaehrStoffeZurueck(HttpServletRequest request, HttpServletResponse response)
      throws ServletException, IOException {
    HttpSession session = request.getSession(true);
