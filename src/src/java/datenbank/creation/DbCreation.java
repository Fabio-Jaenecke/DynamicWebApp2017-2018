...
public class DbCreation implements DbCreationInterface {
  ...
  public void setUpTableZugriffsskala() {
    ...
    try {
      ...
      statement.execute("CREATE TABLE zugriffsskala(zindex int(3) primary key, platzierung int(3)," + " aZugriffe int(3));");
      statement.close();
      conn.getConn().commit();
    } catch (SQLException e) {
      LOGGER.log(Level.SEVERE, errorMessage, e);
  ...
  public void setUpTableEnthaelt() {
    ...
    try {
      ...
      statement.execute("CREATE TABLE enthaelt(kindex int(4), nname varchar(100), primary key(kindex, nname),"
          + " foreign key(kindex) references lebensmittelkategorie, " + "foreign key(nname) references naehrstoff);");
      statement.close();
      conn.getConn().commit();
    } catch (SQLException e) {
      LOGGER.log(Level.SEVERE, errorMessage, e);
